...
def seq_script(driver,tevent,pathT,induval):
    ...
    try:
        for info in range(len(infor)):
            ...
            if T_info[0] =="" and T_info[2] == "click":
                ...
                if temp[0] == "xpath": # 对象类型
                    ...
                    if tt == "T23":  #t33
                        ...
                        case.link_text(driver)
                    else:
                        case.click_link(driver)
                elif temp[0] == "name":
                    if tt =="T96" :
                        case.dologin(driver, "teacher", "test")
                    elif tt == "T125":
                ...
                if temp[0] == "xpath" and len(t) == 1:  #最好click对象都是xpath的，看t的长度知道有几个参数，从而调用哪个脚本
                    ...
                    if "T97" in tt:
                        case.down_click_name_one_T97(driver, t[0], temp[1])
                    else:
                    ...
                    if tt == "T29" or tt == "T37": #T44, T52要求密码相同
                        ...
                        case.click_event_input_four_teacher(driver, t[0], t[1], t[2], t[3], induval[j - 4], induval[j - 3],induval[j - 2], induval[j - 1],
                                                    temp[1])
            ...
            generate_seq_ga_check.updateCuriosity(tt)
    except:
        pass
        ...
        for info in range(len(infor)):
            ...
            if T_info[0] =="" and T_info[2] == "click":
                ...
                if temp[0] == "name" and len(t) == 1:
                    driver.switch_to_frame("mainFrame")
                    if tt=="T62": #数据恢复
                        url = "/home/wushumei/python/phpcss_graphTraversal/support/20180921_all.sql"
                        case.T62(driver)
                    else:
                        url="/home/wushumei/python/phpcss_graphTraversal/support/888.png" #上传
...
def runcase(tevent, pathT, induval):
    ...
    if driverflag==0:
        ...
        driver = webdriver.Firefox(options=options)  #webdriver.Firefox(firefox_binary=binary)
